...
const assetPath = path => (isEnglish ? "../" : "") + path;
const slideTitle = title => title.replaceAll("\n", isEnglish ? " " : "");
const languageSwitch = document.querySelector("#languageSwitch");
const slides = window.DECK_CONTENT;
const esc = v => String(v ?? "").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]));
const lines = v => esc(v).replace(/\n/g, "<br>");
const pad = n => String(n).padStart(2,"0");
// SVG paths keep decorative symbols independent of platform emoji fonts.
const icon = name => '<svg class="icon icon--'+name+'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" aria-hidden="true" focusable="false">'+(name==='asterisk'?'<path d="M12 2v20M2 12h20M5 5l14 14M5 19L19 5"/>':'<path d="M5 19L19 5M5 5h14v14"/>')+'</svg>';
const link = s => s ? '<a class="source-link" href="'+esc(s.url)+'" target="_blank" rel="noopener noreferrer">'+esc(s.label)+' '+icon('arrow')+'</a>' : "";
...
const renders={
  cover:(s,i)=>page(s,i,
    '<div class="cover-type"><p class="cover-host">'+ui('AI郵報 presents','AIPost presents')+'</p><h1><span class="openlab">OPEN LAB</span><span class="cover-year">2026<span class="asterisk">'+icon('asterisk')+'</span></span></h1><p class="deck-label">'+esc(s.label)+'</p></div>'+waves()+
    '<div class="cover-bottom"><div><h2>'+lines(s.headline)+'</h2><p>'+esc(s.description)+'</p></div><div class="cover-spec"><strong>'+esc(s.date)+'</strong><div class="city-labels">'+s.cities.map(x=>'<span>'+esc(x)+'</span>').join("")+'</div><p>'+ui('AI 應用體驗・產業分享・創作者交流','AI experiences · Industry insights · Creator connections')+'</p></div></div>',"dark"),
  story:(s,i)=>page(s,i,heading(s)+'<div class="story-grid">'+s.items.map((x,j)=>'<article><span class="mono">0'+(j+1)+'</span><h3>'+esc(x[0])+'</h3><h4>'+esc(x[1])+'</h4><p>'+esc(x[2])+'</p></article>').join('')+'</div><p class="story-closing">'+esc(s.closing)+'</p>'+footer(s)),
  network:(s,i)=>page(s,i,heading(s)+'<div class="network-grid">'+s.items.map(x=>'<article><span class="status-tag">'+esc(x[1])+'</span><h3>'+esc(x[0])+'</h3><p>'+esc(x[2])+'</p></article>').join('')+'</div><p class="fact-strip">'+esc(s.closing)+'</p>'+footer(s),'dark'),
  experience:(s,i)=>page(s,i,heading(s)+'<div class="experience-grid"><div class="editorial-list">'+s.items.map((x,j)=>'<article><span>'+pad(j+1)+'</span><div><h3>'+esc(x[0])+'</h3><p>'+esc(x[1])+'</p></div></article>').join("")+'</div><figure><img src="'+esc(assetPath(s.image))+'" alt="'+ui('AI 生成的 OPEN LAB 活動情境模擬：開放展示區、創作長桌與分享舞台，非實際場地','AI-generated OPEN LAB concept showing exhibition areas, creation tables and a stage; not an actual venue')+'" loading="lazy"><figcaption>'+esc(s.caption)+'</figcaption></figure></div><p class="fact-strip">'+esc(s.facts)+'</p>'),
  ...
  proof:(s,i)=>page(s,i,heading(s)+'<div class="proof-grid">'+s.cases.map(x=>'<figure><img loading="lazy" src="'+esc(assetPath(x.image))+'" alt="'+esc(x.name)+ui(' 過往活動照片',' — past event photograph')+'"><figcaption><span>'+esc(x.kind)+'</span><h3>'+esc(x.name)+'</h3><p>'+esc(x.description)+'</p></figcaption></figure>').join("")+'</div>'+footer(s)),
  cases:(s,i)=>page(s,i,heading(s)+'<div class="case-list">'+s.cases.map((x,j)=>'<a href="'+esc(x.url)+'" target="_blank" rel="noopener noreferrer"><span class="case-no">'+pad(j+1)+'</span><div><small>'+esc(x.category)+'</small><h3>'+esc(x.name)+'</h3></div><p>'+esc(x.description)+'</p><span aria-hidden="true">'+icon('arrow')+'</span></a>').join("")+'</div><div class="partner-names">'+s.partners.map(x=>'<span>'+esc(x)+'</span>').join("")+'</div>'+footer(s),"dark"),
  ...
  challenges:(s,i)=>page(s,i,heading(s)+'<div class="challenge-grid">'+s.competitions.map((x,j)=>'<article><span class="mono">CHALLENGE 0'+(j+1)+'</span><h3>'+esc(x.name)+'</h3><h4>'+esc(x.subtitle)+'</h4><p class="example">'+esc(x.example)+'</p><div class="challenge-role"><span>'+ui('品牌可參與','How your brand can take part')+'</span><p>'+esc(x.role)+'</p></div></article>').join("")+'</div><p class="fact-strip">'+esc(s.strip)+'</p>',"dark"),
  visibility:(s,i)=>page(s,i,heading(s)+'<div class="channel-strip">'+s.channels.map(x=>x.url?'<a href="'+esc(x.url)+'" target="_blank" rel="noopener noreferrer" aria-label="'+esc(x.label)+ui('（另開分頁）',' (opens in a new tab)')+'">'+esc(x.label)+' '+icon('arrow')+'</a>':'<span>'+esc(x.label)+'</span>').join("")+'</div><div class="visibility-list">'+s.items.map((x,j)=>'<article><span>'+pad(j+1)+'</span><h3>'+esc(x[0])+'</h3><p>'+esc(x[1])+'</p></article>').join("")+'</div>'+footer(s),"lime"),
  partnerships:(s,i)=>page(s,i,heading(s)+'<div class="table-wrap"><table><thead><tr><th>'+ui('合作方式','Partnership')+'</th><th>'+ui('你帶來','Your contribution')+'</th><th>'+ui('對應權益','What is included')+'</th></tr></thead><tbody>'+s.rows.map(x=>'<tr>'+x.map((v,j)=>j===0?'<th scope="row">'+esc(v)+'</th>':'<td>'+esc(v)+'</td>').join("")+'</tr>').join("")+'</tbody></table></div><p class="package-common">'+esc(s.rule)+'</p><div class="resource-examples">'+s.examples.map(x=>'<article><h3>'+esc(x[0])+'</h3><p>'+esc(x[1])+'</p></article>').join("")+'</div>'+footer(s)),
  scope:(s,i)=>page(s,i,heading(s)+'<div class="table-wrap benefit-wrap" tabindex="0" role="region" aria-label="'+ui('單場贊助權益比較，可橫向捲動','Sponsorship benefits per event; scroll horizontally to compare')+'"><table class="benefit-table"><thead><tr><th scope="col">'+ui('單場合作權益','Benefits per event')+'</th>'+s.tiers.map(x=>'<th scope="col">'+esc(x[0])+'<strong>NT$ '+esc(x[1])+'</strong></th>').join("")+'</tr></thead><tbody>'+s.benefits.map(x=>'<tr><th scope="row">'+esc(x[0])+'</th>'+x.slice(1).map(v=>'<td>'+esc(v)+'</td>').join("")+'</tr>').join("")+'</tbody></table></div><p class="package-common">'+esc(s.common)+'</p>'+footer(s)),
  contact:(s,i)=>page(s,i,heading(s)+'<div class="contact-grid"><div class="event-stamp"><strong>OPEN<br>INVITATION<span>'+icon('arrow')+'</span></strong><p>'+esc(s.cities)+'<br>'+esc(s.date)+'</p></div><address><span>'+esc(s.name)+'</span><p>'+esc(s.org)+'</p><a class="email" href="mailto:'+esc(s.email)+'">'+esc(s.email)+' '+icon('arrow')+'</a><a href="tel:+886928910214">'+esc(s.phone)+'</a>'+link(s.link)+'</address></div>'+footer(s),"dark")
};
...
dots.innerHTML=slides.map((s,i)=>'<button aria-label="'+ui('第 ','Slide ')+(i+1)+ui(' 頁：',': ')+esc(slideTitle(s.title))+'" data-index="'+i+'"></button>').join("");
document.querySelector("#overviewGrid").innerHTML=slides.map((s,i)=>'<button data-index="'+i+'"><span>'+pad(i+1)+'</span><strong>'+esc(slideTitle(s.title))+'</strong><small>'+esc(s.id)+'</small></button>').join("");
function update(index,hash=true){
  current=index;
  if(languageSwitch) languageSwitch.href=(isEnglish ? "../" : "en/")+"#"+slides[current].id;
  counter.textContent=pad(current+1)+" / "+pad(slides.length);
  currentTitle.textContent=slideTitle(slides[current].title);
