...
const renders={
  cover:(s,i)=>page(s,i,
    '<div class="cover-type"><p class="cover-host">AI郵報 presents</p><h1><span class="openlab">OPEN LAB</span><span class="cover-year">2026<span class="asterisk">'+icon('asterisk')+'</span></span></h1><p class="deck-label">'+esc(s.label)+'</p></div>'+waves()+
    '<div class="cover-bottom"><div><h2>'+lines(s.headline)+'</h2><p>'+esc(s.description)+'</p></div><div class="cover-spec"><strong>'+esc(s.date)+'</strong><div class="city-labels">'+s.cities.map(x=>'<span>'+esc(x)+'</span>').join("")+'</div><p>AI 應用體驗・產業分享・創作者交流</p></div></div>',"dark"),
  ...
  visibility:(s,i)=>page(s,i,heading(s)+'<div class="channel-strip">'+s.channels.map(x=>x.url?'<a href="'+esc(x.url)+'" target="_blank" rel="noopener noreferrer" aria-label="'+esc(x.label)+'（另開分頁）">'+esc(x.label)+' '+icon('arrow')+'</a>':'<span>'+esc(x.label)+'</span>').join("")+'</div><div class="visibility-list">'+s.items.map((x,j)=>'<article><span>'+pad(j+1)+'</span><h3>'+esc(x[0])+'</h3><p>'+esc(x[1])+'</p></article>').join("")+'</div>'+footer(s),"lime"),
